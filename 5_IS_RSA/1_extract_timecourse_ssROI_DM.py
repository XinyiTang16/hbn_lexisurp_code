"""
Extract ROI time series and phase angles for movie-watching fMRI data (DM).

This script extracts ROI mean time series from the top 10% of voxels within each ROI based on subject-specific lexical surprisal encoding maps. 
...
# ---------------------------- Paths and Setup ----------------------------
DATA_dir = "movieDM_subdata/"
subjects = sorted([f.split('_')[0] for f in os.listdir(DATA_dir) if f.endswith('.hdf')])
print("Total number of subjects:", len(subjects)) 
print("Subjects:", subjects[:10])  # Print first 10 subjects for verification

# Feature predict files for each subject
CON = "GPT3_surprisal"
...
contrast_map_dict = {f.split("/")[-1].split("_")[0]: f for f in contrast_files}
print("Total number of contrast map:", len(contrast_map_dict))
print("Contrast map key:", list(contrast_map_dict.keys())[:10])  # Print first 10 contrast map files for verification

# Brain mask and ROI setup
...
ROIs = sorted(glob(os.path.join(ROI_dir, 'rROI*CoreLanguage*.nii'))) #language network ROIs
...
def load_subject_data(subject_id):
    fpath = os.path.join(DATA_dir, f"{subject_id}_movieDM_wholebrain.hdf")
    with h5py.File(fpath, 'r') as f:
        return f["fmri_response"][:].astype(np.float32)  # (T, V)

def extract_top_voxel_indices(subject_contrast_path, roi_path):
...
def compute_top_voxel_ts_parallel(subjects, roi_names, n_jobs):
    ...
    def process_subject(s_idx, sub):
        ...
        for r_idx, roi in enumerate(roi_names):
            ...
            voxels = extract_top_voxel_indices(contrast_path, ROI_dict[roi])
    ...
    results = Parallel(n_jobs=n_jobs)(
        delayed(process_subject)(s_idx, sub) for s_idx, sub in enumerate(tqdm(subjects, desc="Extracting top 10perc voxel TS"))
    )
...
# ------------------------------ Run Pipeline -----------------------------
out_dir = "5_movieDM_ISRSA/1_sstop10perc_timecourse/"
...
for net_name, roi_list in network_dict.items():
    ...
    np.save(os.path.join(net_out_dir, f'movieDM_{net_name}_sstop10perc_ROI_meantimecourse.npy'), sstop_ts_data)
    
    phase_data = compute_phase_angles(sstop_ts_data)
    T, R, S = phase_data.shape
    print("phase.shape", phase_data.shape) # (T, R, S)
    np.save(os.path.join(net_out_dir, f'movieDM_{net_name}_sstop10perc_ROI_meantimecourse_phase.npy'), phase_data)
